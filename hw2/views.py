...
def url_zipf(request):
    ...
    if request.method == 'POST':
        ...
        file_text = scrape_texts
        word_counts = prepared_words_counter(scrape_texts,stopwords=False,stem=False)        # bag_of_words
        all_word_counts = top_n_words(word_counts,n=int(request.POST['ranks'])) # 前n個關鍵字(未刪除stopwords)
        all_freq_list = word_freq_list(word_counts)
        word_counts = prepared_words_counter(scrape_texts) 
        pre_word_counts = top_n_words(word_counts,n=int(request.POST['ranks']))  # 前n個關鍵字(前處理)
        pre_freq_list = word_freq_list(word_counts)
        keyword = textCheck(request.POST['keyword'],[x[0] for x in prepared_words_counter(show_texts,stopwords=True,stem=False)]) #�ϥΧR��stopwords�B��stem���A������J��keywords
        #print(keyword)
        #print(request.POST['keyword'])
...
def scrape(url,sleep = 1): # 從網頁擷取內容
